...
class _LoginRoute(Route):
    ...
    >>> from purestorage_checkmk_test.flasharray.mock_apitokens_container import APITokensContainer, APITokenObject, APIToken
    >>> import time
    >>> auth_token_storage = _AuthTokenStorage()
    >>> route = _LoginRoute(APITokensContainer({"asdf"}), auth_token_storage))

    You can now use auth_token_storage for authenticating requests.
    """

    path = re.compile("^/api/2.21/login$")
